...
class Annotator(PyQt5.QtWidgets.QWidget):
    ...
    def __init__(self, size=None, resize_size=128):
        ...
        # accounts for resizing of the widget and for zooming in the part of the image
        self.zoomFactor = 1
        # padding when aspect ratio of image and widget does not match
        self.padding = PyQt5.QtCore.QPoint(0, 0)
        # part of the target being drawn on
        ...
        # offset between image center and area of interest center
        ...
        self.hPressed = False
        self.textField.setAttribute(
            PyQt5.QtCore.Qt.WA_TransparentForMouseEvents)
        ...
        self.oldAnn, self.oldResize = [self.annotationPix.copy()], [
            self.resizePix.copy()]
    ...
    def fromFolder(cls, folder_name, net_name, instant_seg=False, negative_skeleton=False,
        ...
        net = AbstractUNet(args).to(device)
        ckpt = torch.load(name, map_location=lambda storage, loc: storage, weights_only=False)
        net.load_state_dict(ckpt["net"])
    ...
    def reset_masks(self):
        self.masks_pix = PyQt5.QtGui.QPixmap(
    ...
    def reset_image(self):
        ...
        self.annotationPix = PyQt5.QtGui.QPixmap(self.imagePix.width(),
                                                 self.imagePix.height())
        self.annotationPix.fill(self.color_picker(label=0, opacity=0))
        self.resizePix = PyQt5.QtGui.QPixmap(
        ...
        self.annotationsFilename = os.path.join(
            "annotations", os.path.basename(im_name).replace('jpg', 'png'))
    ...
    def makePainter_resize(self, pixmap, color):
        ...
        # a compromise between odd and even pen width
    ...
    def paintEvent(self, event):
        ...
        painter_display = PyQt5.QtGui.QPainter(
            self)  # this is painter used for display
    ...
    def drawCursorPoint(self, point):
        ...
        self.cursorPix.fill(self.color_picker(
            label=0, opacity=0))  # transparent
    ...
    def adjustTarget(self):
        ...
        self.cursorPix.fill(self.color_picker(
            label=0, opacity=0))  # clear (fill with transparent)
        self.update()
    ...
    def draw_masks(self, masks, un_col):
        ...
        if n_masks == 1:
            ...
            qimage = qimage2ndarray.array2qimage(
                (masks == 1)[:, :, np.newaxis]*colors[label_num], normalize=True)
            imagePix = PyQt5.QtGui.QPixmap.fromImage(qimage)

            # imagePix.fill(self.color_picker(label=0, opacity=0))
            mask = imagePix.createMaskFromColor(
                PyQt5.QtGui.QColor("black"), PyQt5.QtCore.Qt.MaskInColor)
            ...
            for i in range(n_masks):
                ...
                #                         (self.rect().bottomRight() - self.padding).y()+1))
                mask = np.hstack(
                    (z_w, mask, z_w)) if self.padding.x() else mask
                mask = np.vstack(
                    (z_h, mask, z_h)) if self.padding.y() else mask
                qimage = qimage2ndarray.array2qimage(
                    (mask)[:, :, np.newaxis]*colors[label_num], normalize=True)
    ...
    def predict(self):
        ...
        batch = torch.zeros((len(un_col), 4, self.resize_size,
                            self.resize_size)).to(self.device)
        for i, col in enumerate(un_col):
            skelet = (pixmap == col).all(-1).astype(np.float32)
            if self.negative_skeleton:
                skelet[np.logical_and(
                    pixmap.sum(-1) != 0, (pixmap != col).any(-1))] = -1
            ...
            batch[i] = torch.cat((im, skelet[None, ...]),
